# Nami — Codex Animated Pet

<p align="center">
  <img src="screenshots/nami-idle.png" alt="Nami idle pose" width="160">
</p>

Nami is a relaxed summer chibi girl with a deep-navy bob, aqua eyes, a straw bucket hat, white sandals, and a short mint sleeveless A-line dress. The dress keeps several soft vertical pleats that move naturally throughout the animation set. She is packaged as a Codex sprite v2 pet with nine standard animation states and sixteen clockwise look directions.

나미는 짙은 남청색 단발과 아쿠아색 눈, 밀짚 버킷햇, 흰색 샌들, 짧은 민트색 민소매 A라인 원피스가 특징인 여름 분위기의 치비 캐릭터입니다. 원피스에는 여러 개의 부드러운 세로 플리츠가 들어가며 동작에 맞춰 자연스럽게 움직입니다. Codex sprite v2 규격의 아홉 가지 기본 애니메이션과 열여섯 방향 시선을 지원합니다.
...
![Nami animation sheet](screenshots/animation-sheet.png)
...
![Nami look directions](screenshots/look-directions.png)
...
```bash
mkdir -p "$HOME/.codex/pets/nami"
cp "Nami/pet.json" "$HOME/.codex/pets/nami/pet.json"
cp "Nami/spritesheet.webp" "$HOME/.codex/pets/nami/spritesheet.webp"
```

Restart or refresh the Codex desktop app if Nami does not appear immediately.

To uninstall:

```bash
rm -rf "$HOME/.codex/pets/nami"
```
...
- The two near-down intermediate horizontal cues were subtle in blind isolation and accepted after labeled ordered-loop review
...
When sharing or adapting Nami's visual assets, use this attribution where practical:
